...
const Proyecto = () => {
	...
	const [proyecto, setProyecto] =  useState({nombre:'Proy1', img:'https://i.imgur.com/VGEVWUy.png'})
	const { globalState, globalDispatch } = useContext(MainContext)

	class miCabecera extends Component {
		constructor(props) {
			super(props);
	...
	const init = async () => {
		...
		/* Para la lista */
		const { data } = await ApiRequest().get('/proy1')
		//const { data } = await ApiRequest().post('/pisos', { proyecto: proyecto })
	...
	const onDelete = async (id) => {
		...
		if(sign == 'si' ){
			try {
				const { data } = await ApiRequest().post('/proy1eliminar', { id: id })
				setMensaje({
	...
	const onSubmit = async () => {
		try {
			const { data } = await ApiRequest().post('/proy1guardar', body)
			handleDialog()
	...
	const onEdit = async () => {
		try {
			const { data } = await ApiRequest().post('/proy1editar', body)
			handleDialog()
			setBody(initialState)
			setMensaje({
				ident: new Date().getTime(),
	  ...
	  <>
		<Dialog maxWidth='xs' open={openDialog} onClose={handleDialog} className={classes.fondoventana} >
				...
				<DialogContent className={classes.fondo} >
					<Grid container spacing={2}  className={classes.dialogos} >
		...
		<CRow>
		  <CCol xs>
				...
				<Container maxWidth='100%' >
					{/* <Box sx={{ pb: 5 }}>
						<Typography variant="h5">Lista de Pisos de Proy 1</Typography>
					</Box> */}
