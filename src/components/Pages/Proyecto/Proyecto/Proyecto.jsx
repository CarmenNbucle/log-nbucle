...
const Proyecto = () => {
	...
	const onDelete = async (id) => {
		try {
			const { data } = await ApiRequest().post('/proyectoeliminar', { id: id })
			setMensaje({
	...
	const onSubmit = async () => {
		try {
			const { data } = await ApiRequest().post('/proyectoguardar', body)
			handleDialog()
	...
	const onEdit = async () => {
		try {
			const { data } = await ApiRequest().post('/proyectoeditar', body)
			handleDialog()
			setBody(initialState)
			setMensaje({
				ident: new Date().getTime(),
		...
		<CRow>
		  <CCol xs>
				...
				<Container maxWidth='100%' >
					<Box sx={{ pb: 5 }}>
						{/* <Typography variant="h5">Lista de Pisos de Proyecto</Typography> */}
					</Box> 
					<Grid container spacing={2}>
